...
public class RawBrotenitaCluster extends AmethystClusterBlock implements Waterloggable {
    ...
    public RawBrotenitaCluster(int height, int xzOffset, Settings settings) {
    ...
    public BlockState getStateForNeighborUpdate(
            BlockState state, Direction facing, BlockState neighborState, WorldAccess worldIn, BlockPos currentPos, BlockPos facingPos
    ) {
        if (state.get(WATERLOGGED)) {
            worldIn.scheduleFluidTick(currentPos, Fluids.WATER, Fluids.WATER.getTickRate(worldIn));
        }

        return facing == (state.get(FACING)).getOpposite() && !state.canPlaceAt(worldIn, currentPos)
                ? Blocks.AIR.getDefaultState()
                : super.getStateForNeighborUpdate(state, facing, neighborState, worldIn, currentPos, facingPos);
    }
