...
import com.example.examplemod.infection.PlayerInfection;
...
public class ModEvents {
    ...
    public void onAttachCapabilities(AttachCapabilitiesEvent<Player> event) {
        if (event.getObject() != null) {
            event.addCapability(new ResourceLocation(ExampleMod.MODID, "time_in_biome"), new PlayerInfectionProvider());
        }
    }
    ...
    public static void onAttachCapabilitiesPlayer(AttachCapabilitiesEvent<Entity> event) {
        if (event.getObject() instanceof Player) {
            if (!event.getObject().getCapability(PlayerInfectionProvider.TIME_IN_BIOME).isPresent()) {
                event.addCapability(new ResourceLocation(ExampleMod.MODID, "properties"), new PlayerInfectionProvider());
            }
        }
    }
